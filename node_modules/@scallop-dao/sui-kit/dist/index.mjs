...
// src/index.ts
import { TransactionBlock as TransactionBlock3, SUI_CLOCK_OBJECT_ID, SUI_SYSTEM_STATE_OBJECT_ID as SUI_SYSTEM_STATE_OBJECT_ID2 } from "@mysten/sui.js";
...
import { FaucetRateLimitError, assert, FaucetResponse } from "@mysten/sui.js";
...
var requestFaucet = async (address, provider) => {
  ...
  const resp = await retry(() => provider.requestSuiFromFaucet(address, headers), {
    backoff: "EXPONENTIAL",
  ...
  makeMoveVec(args, type) {
    ...
    if (type === "object" || !type && defaultSuiType === "object") {
      const objects = args.map(
        (arg) => typeof arg === "string" ? this.txBlock.object(normalizeSuiObjectId(arg)) : arg
      );
...
export {
  ...
  SuiRpcProvider,
  SuiTxBlock,
  TransactionBlock3 as TransactionBlock
};
